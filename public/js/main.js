...
async function submitActivity(event) {
    ...
    try {
        ...
        const responseData = await response.json();  // Get the response from the server
        if (editingIndex === null) {
            activities.push(responseData.activity); // Add new activity to the list
        } else {
            activities[editingIndex] = responseData.activity; // Update the existing activity
        }
...
async function fetchActivities() {
    try {
        const response = await fetch('/getActivities'); // Make sure this route is correct
        if (!response.ok) {
            throw new Error('Failed to fetch activities');
        }

        const data = await response.json(); // Parse JSON response
        console.log('Fetched activities:', data); // Debug the response
        activities = data;  // Update your activities array with the fetched data
        showActivityList();  // Refresh the list after fetching
    } catch (error) {
...
window.onload = function() {
    ...
    document.getElementById('check-list').addEventListener('click', function() {
